...
> Last updated: 2026-07-08 (Frontend-MS06 Done)
...
| Chat UI | `chat_screen.dart` | Done — real sendMessage(), deposit-rejection warnings (MS02b), Status-Icons pending→sent(↑)→routed(✓)→delivered(✓✓)/failed(✗) (MS06) |
...
| Database (Drift v13) | `database.dart` | Done — Channel-Schema v3 (Ed25519 K_auth), destruktive MS03-Migration; v10 (MS03b): `Channels.ratchetState`; v11 (MS04): `Peers.encryptionPublicKey`; v12 (MS05, nicht destruktiv): `session_tags` + `Channels.pendingRgb`; v13 (MS06, nicht destruktiv): `node_scores`; message_id (Dedup), retry_count, last_retry_at, last_cursor |
...
| Message sync service | `message_sync_service.dart` | Done — Dedup-Persist, Cursor/Expiry-Persistenz, OH-Restore beim Start, Ratchet-State-Persist/-Restore (MS03b), Garlic-Session-Persist/-Restore (MS05); R-ACK→routed / Channel-ACK→delivered / Timeout→requeue + Node-Score-Persist/-Restore (MS06) |
